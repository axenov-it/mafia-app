...
export const SettingForm = ({ onSubmit, onCountGamersChange, onPresetChange, gamerItems, presets }: Props) => {
  const { errors, values, touched, handleChange, handleSubmit, handleBlur } = useForm({
    onSubmit
  });
      ...
      >
        ...
        >
          ...
          >
            ...
            <FormHelperText error> {touched.countGamers && errors.countGamers}</FormHelperText>
          </FormControl>
        ...
        {values.countGamers && <FormControl
          css={css`
                margin-top:20px;
                width: 100%;
               `}
        >
          ...
          <FormHelperText error> {touched.preset && errors.preset}</FormHelperText>
        </FormControl>}
      </div>
    </form >
  );
